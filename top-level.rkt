...
(define (top-level e)
  ...
  (define (T e)
    (match e
      ...
      [`(string-ref ,str ,n)
       (if (< n 0)
           (error "string-ref must take value greater than 0")
           (let helper ([str (top-level str)] [n n])
             (match str
               [`(string ,head . ,tail)
                (if (zero? n)
                    head
               ...
               [`(,head . ,tail)
                (if (zero? n)
                    head
                    (helper tail (- n 1)))])))]
      [`(string-append ,(? mystring? str)) str]
      [`(string-append ,(? mystring? strs) ...)
       `(string ,@(flatten-string strs))]
      [`(substring ,str ,start ,end)
       (let helper ([start start] [acc '()] [str (top-level str)])
         (if (eq? start end)
             `(string ,@acc)
             (helper (+ start 1) (append acc `(,(top-level `(string-ref ,str ,start)))) str)))]
