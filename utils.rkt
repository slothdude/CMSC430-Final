...
(define (flatten-string s)
  (match s
    ['() '()]
    [`((string ,e0s ...) ,e1s ...)
     `(,@(flatten-string e0s) ,@(flatten-string e1s))]
    [`(,e0 ,e1s ...)
     `(,e0 ,@(flatten-string e1s))]))
...
(define (racket-compile-eval e)
  (define (rewrite-match e)
    ...
    (match e [`(match ,e0 ,clauses ... (,pat0 ,es ...))
             ...
             [else e]))
  (with-handlers (;[exn:fail:contract:divide-by-zero (lambda (x y) (pretty-print "Evaluation failed: divide by zero") (pretty-print x) (pretty-print e) (error 'divide_by_zero))]
